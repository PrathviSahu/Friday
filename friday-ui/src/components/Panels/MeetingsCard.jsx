      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        ...
        {lastMeeting && (
          <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 10, padding: 12, marginBottom: 10 }}>
            ...
            {lastMeeting.action_items?.length > 0 && (
              <button onClick={() => pushTodos(lastMeeting.id)} disabled={pushingTodos === lastMeeting.id} style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 8, color: ACCENT, cursor: 'pointer', fontSize: 10, textTransform: 'uppercase' }}>
        ...
        {tab === 'meetings' && (
          <>
            ...
            {meetings.map((m) => (
              <div key={m.id} style={{ padding: '10px', marginBottom: 6, background: 'rgba(96,165,250,0.03)', border: '1px solid rgba(96,165,250,0.12)', borderRadius: 10 }}>
                ...
                {m.action_items?.length > 0 && (
                  <button onClick={() => pushTodos(m.id)} disabled={pushingTodos === m.id} style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 8, color: ACCENT, cursor: 'pointer', fontSize: 10, textTransform: 'uppercase' }}>
        ...
        {tab === 'new' && (
          <>
            <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '16px 10px', marginBottom: 10, border: '1px dashed rgba(96,165,250,0.35)', borderRadius: 10, cursor: 'pointer', color: ACCENT, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              <Upload size={13} />
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 0', background: pasteText.trim() && !processing ? ACCENT : 'rgba(96,165,250,0.15)',
