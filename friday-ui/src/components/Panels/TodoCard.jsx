    ...
    if (!isVisible) {
        return (
            <motion.div
                ...
                style={{ background: '#1e293b', border: '1px solid #475569', color: '#cbd5e1', boxShadow: '0 8px 24px rgba(0,0,0,0.3)' }}
            >
        ...
        <AnimatePresence>
            ...
            >
                ...
                <div
                    ...
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 10px', borderBottom: '1px solid #334155', cursor: 'grab', position: 'relative', zIndex: 40 }}
                    className="active:cursor-grabbing"
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'grab' }}>
                        <GripHorizontal size={13} style={{ color: '#475569' }} />
                        <CheckSquare size={15} style={{ color: '#94a3b8' }} />
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0', letterSpacing: '-0.01em' }}>Tasks</span>
                        {totalCount > 0 && (
                            <span style={{ fontSize: 10, background: '#475569', color: '#cbd5e1', borderRadius: 8, padding: '1px 7px', fontWeight: 600 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        {doneCount > 0 && (
                            <motion.button
                                ...
                                style={{ fontSize: 10, background: '#334155', border: '1px solid #475569', color: '#94a3b8', padding: '3px 8px', borderRadius: 6, cursor: 'pointer' }}
                            >
                                Clear done
                            </motion.button>
                        )}
                    ...
                    {['all', 'active', 'done'].map(f => (
                        <button
                            ...
                            style={{
                                ...
                                background: filter === f ? '#475569' : 'transparent',
                                color: filter === f ? '#cbd5e1' : '#475569',
                ...
                {/* ── Todo List ── */}
                <div style={{ maxHeight: 260, overflowY: 'auto', padding: '8px 12px', scrollbarWidth: 'thin', scrollbarColor: '#475569 transparent' }}>
                    <AnimatePresence>
                        {filtered.length === 0 ? (
                            <motion.div
                                initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                style={{ textAlign: 'center', padding: '24px 0', color: '#475569', fontSize: 12 }}
                            >
                            ...
                            <motion.div
                                ...
                                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', borderRadius: 10, marginBottom: 4, background: '#1e293b', border: '1px solid #334155', cursor: 'default' }}
                            ...
                            >
                                ...
                                {editId === todo.id ? (
                                    <input
                                        ...
                                        style={{ flex: 1, background: '#334155', border: '1px solid #475569', borderRadius: 5, padding: '2px 6px', color: '#e2e8f0', fontSize: 12, outline: 'none' }}
                                    />
                                ) : (
                                    <span
                                        style={{ flex: 1, fontSize: 12, color: todo.done ? '#475569' : '#c7d2fe', textDecoration: todo.done ? 'line-through' : 'none', cursor: 'text', transition: 'all 0.2s' }}
                                ...
                                >
                                    <button onClick={() => { setEditId(todo.id); setEditText(todo.text); }}
                                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', padding: 3, display: 'flex' }}
                                        onMouseEnter={e => e.currentTarget.style.color = '#94a3b8'}
                                        onMouseLeave={e => e.currentTarget.style.color = '#475569'}>
                                        <Pencil size={11} />
                                    </button>
                ...
                {/* ── Add Task Input ── */}
                <div style={{ padding: '8px 12px 12px', borderTop: '1px solid #334155' }}>
                    <form onSubmit={addTodo} style={{ display: 'flex', gap: 6 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            <button
                                ...
                                style={{ height: 34, width: 34, borderRadius: 8, background: '#1e293b', border: '1px solid #475569', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                            >
                            ...
                            <AnimatePresence>
                                {showPriority && (
                                    <motion.div
                                        ...
                                        style={{ position: 'absolute', bottom: 38, left: 0, background: '#1e293b', border: '1px solid #475569', borderRadius: 8, padding: '4px', zIndex: 10, minWidth: 90 }}
                                    >
                                        {Object.entries(PRIORITY_META).map(([k, v]) => (
                                            <button key={k} type="button"
                                                onClick={() => { setPriority(k); setShowPriority(false); }}
                                                style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 8px', background: priority === k ? '#334155' : 'none', border: 'none', borderRadius: 5, cursor: 'pointer', color: priority === k ? '#cbd5e1' : '#94a3b8', fontSize: 11, fontWeight: 600 }}
                                            >
                        ...
                        <input
                            ...
                            style={{
                                flex: 1, height: 34, background: '#1e293b', border: '1px solid #475569',
                                borderRadius: 8, padding: '0 10px', color: '#e2e8f0', fontSize: 12, outline: 'none',
                                fontFamily: 'inherit'
                            }}
                        ...
                        <motion.button
                            ...
                            disabled={!input.trim()}
                            style={{ height: 34, width: 34, borderRadius: 8, background: input.trim() ? '#4f46e5' : '#334155', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.2s' }}
                        >
                            <Plus size={16} color={input.trim() ? '#fff' : '#475569'} />
                        </motion.button>
                    </form>
                </div>
